...
const Cart = () => {
    ...
    const itemTotal = cartItems.reduce((acc, item) => {
        return (acc = acc + item.card.info.price / 10000);
    }, 0);
    const total = itemTotal + 5.00 + 8.32;
    return (
        <div>
            <Header />
            <div className="px-30 flex flex-col justify-center items-center bg-gray-100 p-20 text-md">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-1">
                        <div className="bg-gray-50 px-6 py-8 rounded shadow-md">
                            <h2 className="text-md font-semibold mb-4">Account</h2>
                            <p className="mt-1">To place your order now, log in to your existing account or sign up</p>
                            <Link to={'/login'} className="underline text-black font-bold py-4 px-6 mt-2 mr-2">
                                Login / Register
                            </Link>

                        </div>
                        <div className="bg-gray-50 rounded p-6 shadow-md">
                            <h2 className="text-md font-semibold mb-4">Delivery Address</h2>
                            <form className="grid grid-cols-2 gap-4">
                                <div className="mb-4">
                                    <label htmlFor="fullname" className="block text-sm font-medium">Full Name</label>
                                    <input type="text" id="fullname" name="fullname" className="mt-1 p-2 w-full rounded-md bg-gray-200" />
                                </div>

                                <div className="mb-4">
                                    <label htmlFor="address2" className="block text-sm font-medium">Address Line</label>
                                    <input type="text" id="address2" name="address2" className="mt-1 p-2 w-full rounded-md bg-gray-200" />
                                </div>
                                <div className="mb-4">
                                    <label htmlFor="city" className="block text-sm font-medium">City</label>
                                    <input type="text" id="city" name="city" className="mt-1 p-2 w-full rounded-md bg-gray-200" />
                                </div>
                                <div className="mb-4">
                                    <label htmlFor="state" className="block text-sm font-medium">State/Province/Region</label>
                                    <input type="text" id="state" name="state" className="mt-1 p-2 w-full rounded-md bg-gray-200" />
                                </div>
                                <div className="mb-4">
                                    <label htmlFor="zip" className="block text-sm font-medium">ZIP/Postal Code</label>
                                    <input type="text" id="zip" name="zip" className="mt-1 p-2 w-full rounded-md bg-gray-200" />
                                </div>
                                <div className="mb-4">
                                    <label htmlFor="country" className="block text-sm font-medium">Country</label>
                                    <select id="country" name="country" className="mt-1 p-2 w-full rounded-md  text-gray-600 bg-gray-200">
                                ...
                                <div className="col-span-2 mt-2">
                                    <button type="submit" className=" bg-gray-950 hover:bg-blue-700 text-white font-bold rounded py-2 px-4 w-full">Save Address</button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div className=" text-gray-800 ">
                        <section aria-labelledby="summary-heading" className="flex shadow-md flex-col p-6 rounded bg-gray-50 overflow-auto  h-64">
                            <h2 className="text-md font-semibold mb-4">Order Summary</h2>
                            <div className="grid grid-cols-1 gap-4 text-sm">
                                <ItemList items={[...new Set(cartItems)]} />
                                <div div className="flex justify-between w-full mb-2">
                                    <span>Sub Total</span>
                                    <span>${Math.ceil(itemTotal, 2).toFixed(2)}</span>
                                </div>
                            ...
                            <div className="flex sticky justify-between mb-2 border-t border-gray-300 pt-2">
                                <span className="font-semibold">Order total</span>
                                <span className="font-semibold">${Math.ceil(itemTotal, 2) + 5.00 + 8.32}</span>
                            </div>
                        </section>
                        <div className="bg-gray-50 mt-5 rounded">
                            <div class="p-6 shadow-md">
                                <fieldset>
                                    <h2 className="text-md font-semibold mb-4">Payment</h2>
